...
const mockDiscoverBatch = discoverActivitiesBatch as ReturnType<typeof vi.fn>;
...
beforeEach(() => {
  ...
  mockDiscoverBatch.mockResolvedValue([
    {
...
describe("POST /api/v1/trips/:id/discover-activities", () => {
  it("returns 401 for unauthenticated users", async () => {
    ...
    const res = await POST(makeRequest({ cityId: "tokyo", batchIndex: 0 }), {
  ...
  it("returns a validated activity batch for authenticated owners", async () => {
    const res = await POST(makeRequest({ cityId: "tokyo", batchIndex: 0 }), {
      params: Promise.resolve({ id: "trip-1" }),
    });
    const json = await res.json();
    ...
    expect(mockDiscoverBatch).toHaveBeenCalledWith({
      tripId: "trip-1",
      profile: {
        nationality: "German",
        homeAirport: "FRA",
