  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (tripId) {
        ...
        // Step 2: Start SSE generation stream
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-xl mx-auto px-4 pt-24 pb-12">
        ...
        <AnimatePresence mode="wait" custom={1}>
          <motion.div key={planStep} custom={1} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25, ease: "easeInOut" }}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {vibes.map((v) => (
                    <button key={v.id} onClick={() => setVibe(v.id as TripVibe)}
                      className={`p-5 rounded-xl border-2 text-left transition-all duration-200 ${vibe === v.id ? "border-primary bg-primary/5" : "border-border hover:border-primary/40"}`}>
              ...
              <div>
                ...
                <div className="flex items-center justify-center gap-8">
                  <button onClick={() => setTravelers(Math.max(1, travelers - 1))}
                    className="w-12 h-12 rounded-full border-2 border-border flex items-center justify-center hover:border-primary hover:bg-primary/5 transition-all text-2xl font-bold text-foreground">−</button>
              ...
              <div>
                ...
                <div className="card-travel bg-background mb-6">
                  {[
                    ...
                    { label: "Dates", value: dateStart && dateEnd ? `${dateStart} → ${dateEnd} (${dayCount} days)` : "Not set" },
                    ...
                    <div key={row.label} className={`flex justify-between py-3 ${i < arr.length - 1 ? "border-b border-border" : ""}`}>
                      <span className="text-muted-foreground">{row.label}</span>
                      <span className="font-medium text-foreground text-right ml-4">{row.value}</span>
                    </div>
                  ))}
                </div>
                <button onClick={handleGenerate} className="btn-primary w-full flex items-center justify-center gap-2 text-base py-4">
        ...
        {planStep < 6 && (
          <div className="mt-8 flex items-center justify-between">
            <button onClick={() => setPlanStep(Math.max(1, planStep - 1))}
              className={`flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground transition-colors ${planStep === 1 ? "opacity-0 pointer-events-none" : ""}`}>
              <ArrowLeft className="w-4 h-4" /> Back
            </button>
            <button onClick={() => setPlanStep(planStep + 1)} disabled={!canAdvance()}
        ...
        {planStep === 1 && (
          <div className="mt-4 text-center">
            <button onClick={() => setPlanStep(2)} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
              Skip for now
            </button>
          </div>
        )}
